...
import sky.spring.transaction_lock.event_with_external_update.facade.EventJoinWithExternalApiUpdateFacade;
...
class EventJoinWithExternalApiUpdateFacadeTest {
    ...
    @Autowired
    private EventJoinWithExternalApiUpdateFacade eventJoinWithExternalApiUpdateFacade;
    @MockitoSpyBean
    ...
    void setUp() {
        ...
        testMembers = EventFixture.createMembers(20);
        ...
        doAnswer(invocation -> {
            ...
            Member testMember = EventFixture.createMember("테스트 유저" + memberId);
            return EventFixture.createTestParticipant(testEvent, testMember);
        }).when(eventJoinService).joinEventWithTransaction(anyLong(),anyLong());
    ...
    void externalApiCallAndDbUpdateMismatchTest() throws InterruptedException {
        ...
        ConcurrentTestUtil.executeConcurrentJoins(
                testEvent.getId(),
                testMembers,
                (eventId, memberId) -> eventJoinWithExternalApiUpdateFacade.joinEvent(eventId, memberId)
        );
